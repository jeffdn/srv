		      ...
		      hash_t * mps)
{
	...
	if (NULL != (mf = (struct _modfunc *)hash_get(mps, req))) {
		DEBUGF(__FILE__, __LINE__,
		       "checking if %s needs a handler...\n", path);

		/* gotta handle this bitch with the function */
		if (NULL !=
		    (resp->data = mf->func(path, &mt, params, param_cnt))) {
			resp->pregen = 1;
			resp->len = mt.len;
			resp->type = mt.ftype;
			resp->code =
			    (mt.status) ? RESP_HTTP_200 : RESP_HTTP_404;
			snprintf(resp->header, sizeof resp->header,
				 "HTTP/1.1 %s %s\r\n" "Connection: close\r\n"
				 "Date: %s\r\n" "Server: srv/0.1.1\r\n"
				 "Content-Length: %lu\r\n"
				 ...
				 (long unsigned)resp->len,
				 mime_types[resp->type][1]);
		} else {
...
char *srv_build_dir_index(const char *dir, file_t * list, unsigned int cnt)
{
	...
	for (i = 0; i < cnt; i++) {
		...
		if (S_ISDIR(f->type)) {
			...
			if (f->size < 1024) {
				snprintf(size, sizeof size, "%lub",
					 (long unsigned)f->size);
			} else if (f->size >= 1024 && f->size < 1048576) {
				snprintf(size, sizeof size, "%.1fk",
					 (float)f->size / 1024);
			} else {
				snprintf(size, sizeof size, "%.1fm",
					 (float)f->size / 1048576);
			}
		}
