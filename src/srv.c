...
int srv_conn_resp_ready(conn_t * clnt)
{
	...
	if (!
	    (sent =
	     send(clnt->sock, clnt->resp.header, clnt->resp.headlen, 0))) {
...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < conf.mod_cnt; ++i) {
		...
		mods[i].func =
		    (_srv_modfunc_t) dlsym(mods[i].mod, conf.mods[i].func);
		...
		for (j = 0; j < conf.mods[i].hnd_cnt; j++) {
			...
			if (conf.mods[i].hnd[j].type == SRV_HANDLER_FILE) {
				...
				DEBUGF(__FILE__, __LINE__,
				       ...
				       conf.mods[i].hnd[j].data,
				       conf.mods[i].func);
				hash_insert(&mps, conf.mods[i].hnd[j].data,
					    &mods[i]);
			}
		}
	}
